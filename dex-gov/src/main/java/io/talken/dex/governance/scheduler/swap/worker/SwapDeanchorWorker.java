...
public class SwapDeanchorWorker extends SwapTaskWorker {
	...
	public void proceed(DexTaskSwapRecord record) {
		...
		try {
			...
			updateRecordException(record, DexSwapStatusEnum.DEANCHOR_FAILED, "build tx", ex);
			return;
		}
		...
		if(!deanchorResult.isSuccess()) {
			updateRecordError(record, DexSwapStatusEnum.DEANCHOR_FAILED, "request deanchor", deanchorResult.getErrorCode(), deanchorResult.getErrorMessage());
			return;
		}
		...
		try {
			...
			if(!txResponse.isSuccess()) {
				...
				updateRecordError(record, DexSwapStatusEnum.DEANCHOR_FAILED, "submit tx", resultCodes.first(), resultCodes.second());
				return;
			}
			...
			updateRecordException(record, DexSwapStatusEnum.DEANCHOR_FAILED, "submit tx", ex);
		}
	}
}
