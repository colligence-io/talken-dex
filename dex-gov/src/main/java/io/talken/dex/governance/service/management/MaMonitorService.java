	...
	private ObjectPair<BigDecimal, BigDecimal> getAccountBalance(TokenMetaTable.Meta meta, String address) throws Exception {
		...
		switch(meta.getBctxType()) {
			...
			case ETHEREUM_ERC20_TOKEN:
				BigInteger rawErc20Bal = contractInfoService.getBalanceOf(ethereumNetworkService.getRpcClient().newClient(), meta.getAux().get(TokenMetaAuxCodeEnum.ERC20_CONTRACT_ID).toString(), address);
				if(rawErc20Bal != null)
			...
			case ETHEREUM:
				BigInteger rawEthBal = ethereumNetworkService.getRpcClient().newClient().ethGetBalance(address, DefaultBlockParameterName.LATEST).send().getBalance();
				if(rawEthBal != null)
